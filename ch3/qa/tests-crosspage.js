...
suite('Cross-Page Tests', function() {
    ...
    test('requesting a group rate from oregon coast tour page should ' +
         'populate referrer field', function(done) {
             var referrer = 'http://localhost:8000/tours/oregon-coast';
             browser.visit(referrer, function() {
                 browser.clickLink('.requestGroupRate', function() {
                     assert(browser.field('referrer').value === referrer);
                     done();
    ...
    test('visiting the "request group rate" page directly should result ' +
         'in an empty referrer field', function(done){
             browser.visit('http://localhost:8000/tours/request-group-rate', function() {
